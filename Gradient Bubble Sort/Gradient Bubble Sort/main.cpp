...
int main()
{
	...
	while (running)
	{
		...
		cout << "2. Mickey Mouse (200 x 200 pixels)" << endl;
		cout << "3. Kitten (300 x 300 pixels)" << endl;
		cout << "4. Quit" << endl;
		cin >> choice;
		if (choice == 1)
		{
		...
		{
			// READ IN KITTEN TO VECTOR OF PIXELS
			selection = "./image_input/Kitten.tga";
			imageName = "Kitten";
		}
		else if (choice == 4)
		{
...
void merge(vector<pixel>& pixelVec, string pixelColor, int left, int mid, int right) {
	...
	if (pixelColor == "Red") { //if the user wants to merge sort by red
		...
		vector<pixel> X; //left
		vector<pixel> Y; //right

		for (int i = 0; i < n1; i++) { //sorting
			X.push_back(pixelVec[left + i]);
		}
		for (int j = 0; j < n2; j++) {
			Y.push_back(pixelVec[mid + 1 + j]);
		}
...
void DisplayImage(string image, string sortMethod)
{
	...
	else if (image == "Mickey Mouse")
	{
		...
		if (sortMethod == "Bubble")
			filePath = "./image_output/bubble_MickeyMouse.tga";
		else
			filePath = "./image_output/merge_MickeyMouse.tga";
	...
	else if (image == "Kitten")
	{
		sf::RenderWindow window(sf::VideoMode(1000, 1000), "Bucharest Gradient");
		sf::Texture gradientTexture;
		string filePath;

		if (sortMethod == "Bubble")
			filePath = "./image_output/bubble_kitten.tga";
		else
			filePath = "./image_output/merge_kitten.tga";
